        ...
        while (true) {
           ...
           // File file = new File("/root/1561101780000");
            ...
            if (collect.size() >0) {
                ...
                long stopTime = System.currentTimeMillis();
                System.out.println(file.getPath()+"文件夹所用时间:" + (stopTime - startTime) + "毫秒");
            }
        }
*/
 ...
 */
     ...
     * @Date: 2019/10/15 13:54
     */
    private static void insulation_overheating(InfluxDBConnection influxDBConnection, File file) {
        String s;
        List<String> records = new ArrayList<String>();
        s = readToString(file.toString());
        String[] split = s.split("\n");
     ...
     */
    private static void bearing_current(InfluxDBConnection influxDBConnection, File file) {
        String s;
        List<String> records = new ArrayList<String>();
        s = readToString(file.toString());
        String[] split = s.split("\n");
...
*/
    private static void ReadInfluxDB(InfluxDBConnection influxDBConnection) {
        ...
        // QueryResult query = influxDBConnection.query("select count(sersor1) from onlinemonitordata_1_tsziot_front_rotor_vibration");
         QueryResult query = influxDBConnection.query("select * from rear_rotor_vibration_1 limit 1");
